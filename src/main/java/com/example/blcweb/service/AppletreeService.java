...
public class AppletreeService {
	...
	public void moveAppleToTrash(long appleId, long loginUserId) {

		AppletreeEntity apple = appletreeRepository.findById(appleId)
				.orElseThrow(() -> new IllegalArgumentException("apple not found"));

		// 「作成者のみ削除可」
		if (apple.getUserId() != loginUserId) {
			throw new IllegalStateException("削除権限がありません。");
		}

		// りんご本体（root）だけ削除対象にしたいなら
		if (apple.getParentId() != null) {
			throw new IllegalStateException("返信メッセージは削除できません。");
		}

		// すでにゴミ箱なら何もしない（好み）
		if (apple.getStatus() == AppleStatus.TRASH) {
			return;
		}

		apple.setStatus(AppleStatus.TRASH);
		appletreeRepository.save(apple);
	}
